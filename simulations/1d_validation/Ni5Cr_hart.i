...
[Kernels]
  ...
  [elec_chem_Ni]
    ...
    variable = phi
    v = w_Ni
    diffusivity = 'chelux_Ni'
  []
  [elec_chem_Cr]
    type = MatDiffusion
    variable = phi
    v = w_Cr
    diffusivity = 'chelux_Cr'
  []
  [elec_phi]
    type = MatDiffusion
    variable = phi
...
[Materials]
  ...
  [D_Cr]
    ...
    function = '( (1-5e-4*f)*D_Cr_V + f*D_Cr_GB)*h_metal  + 500*(1-h_metal) '
    outputs = 'exodus'
  []
  [mobility_Cr]
    type = DerivativeParsedMaterial
...
[Outputs]
  ...
  perf_graph = true
  csv = true
  file_base = 'Ni5Cr_hart/Ni5Cr_hart'
[]
