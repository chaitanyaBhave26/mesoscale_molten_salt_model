...
[MultiApps]
  ...
  [grain_combine_simulation]
    type = FullSolveMultiApp
    execute_on = 'INITIAL'
    input_files = 'microstructure_join.i'
  []
[]
...
[Transfers]
  [from_grain_combine]
    ...
    from_multi_app = grain_combine_simulation
  []
[]
...
[GlobalParams]
  ...
  x_Cr_melt  = 25.0e-6
  x_Ni_melt  = 1e-11#3.758e-11
  E_F = 3.3607
  K = ${fparse 8.6173324e-5 * 973 }

  # coating_thickness = 40
  # x_Cr_coating = 1e-5
...
[ICs]
  ...
  [w_Ni_global_initial]
    ...
    variable = 'w_Ni'
    corners =           '-10.0 -10.0 0.0  '
    opposite_corners =  '150.0  50.0 0.0  '
    inside = '${fparse ${GlobalParams/K} * log ( ${GlobalParams/x_Ni_metal} ) - ${GlobalParams/K} * log( 1 - ${GlobalParams/x_Ni_metal} - ${GlobalParams/x_Cr_metal} ) - 1.73 }'
    outside = '${fparse 2.0 * ${GlobalParams/E_F} + ${GlobalParams/K} * log( ${GlobalParams/x_Ni_melt} ) - 5.158 }'
  []
  [w_Cr_global_initial]
    type = MultiBoundingBoxIC
    variable = 'w_Cr'
    corners =           '-10.0 -10.0 0.0  '
    opposite_corners =  '150.0  50.0 0.0  '
    inside = '${fparse ${GlobalParams/K} * log ( ${GlobalParams/x_Cr_metal} ) - ${GlobalParams/K} * log( 1 - ${GlobalParams/x_Ni_metal} - ${GlobalParams/x_Cr_metal} ) - 1.72 }'
    outside = '${fparse 2.0 * ${GlobalParams/E_F} + ${GlobalParams/K} * log( ${GlobalParams/x_Cr_melt} ) - 6.8029 }'
  []
[]
[BCs]
  [phi_left]
