...
[ICs] #Sets the IC for the second constant phase
  ...
  [grain1_IC]
    type = FunctionIC
    variable = grain1
    function = 'y0:=${Mesh/gen/ymax} / 2;0.5-0.5*tanh(2*(y-y0)/0.2)'
  []
  [grain2_IC]
    type = FunctionIC
    variable = grain2
...
[Materials]
  [constants]
    ...
    prop_values = '1.5     10                   5.1e-5                        0.2           1.1131e-11 '
                  ' 3.141592653 ${fparse 0.025 * 2 / 0.2 } 6.02214076e23 0.0 5e-4 0.8 0.01'
  []
  [energy_constants]
    type = GenericConstantMaterial
