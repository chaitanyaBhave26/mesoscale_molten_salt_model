[Mesh]
  type = GeneratedMesh
  dim = 1
  nx = 640#${fparse int( 6 * 160 / ${GlobalParams/int_width} ) }
  xmax = 160
  ny = 1
  ymax = 2
##  uniform_refine = ${fparse int(log2( ${GlobalParams/l_by_h} ) ) }
[]
...
[GlobalParams]
  ...
  int_width = 2.0
  # l_by_h = 8
  file_base_str = 'high_polynomial/high_polynomial_'
[]
...
[Kernels]
  ...
  [ACBulkF]
    ...
    fa_name = omega_melt
    fb_name = omega_metal
    w = ${fparse 6 * 1.493e7 / ${GlobalParams/int_width}}
    h_name = h_metal
    mob_name = L
    args = 'w_Ni w_Cr'
  []
...
[Materials]
  ...
  [constants]
    ...
    prop_values = '1.5     6.803e6          1.493e7                   ${GlobalParams/int_width}                    1.1131e-11  3.141592653  0.1       6.02214076e23   0.0 5e-4'
  []
  [energy_constants]
    type = GenericConstantMaterial
  ...
  [L]
    ...
    function = '((8*i0*(m/D)^2)/(15*n*F*R*T*interface_thickness_l*1e-6) )*(m3_to_um3/J_to_eV)' #/Va to normalize L for grand potential in eV/atom rather than eV/um3
    outputs = exodus
  []
...
[Outputs]
  ...
  perf_graph = true
  csv = true
  file_base = ${raw ${GlobalParams/file_base_str} ${GlobalParams/int_width} }
[]
