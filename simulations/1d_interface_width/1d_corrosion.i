[Mesh]
  type = GeneratedMesh
  dim = 1
  nx = '${fparse int( ${xmax} / ${GlobalParams/int_width} ) }'
  xmax = 160
  ny = 1
  ymax = 2
  uniform_refine = 2
...
[Kernels]
  ...
  [ACBulkF]
    ...
    fa_name = omega_melt
    fb_name = omega_metal
    w = '${fparse 6 * 1.493e7 / ${GlobalParams/int_width} }' #4.479e7
    h_name = h_metal
    mob_name = L
    args = 'w_Ni w_Cr'
  []
...
[ICs]
  ...
  [w_Cr]
    ...
    outvalue = -0.9699 #Cr2+ atomic fraction is 2.5e-5
    int_width = 0.1
  []
[]
...
[BCs]
  ...
  [w_Ni_right]
    type = DirichletBC
    variable = w_Ni
    value = -0.45065 #Same chemical potential as Ni-5Cr alloy
    boundary = 'right'
  []
  [w_Cr_right]
    type = FunctionDirichletBC
    variable = w_Cr
    function = '-0.9699' #Cr2+ atomic fraction is 2.5e-5
    boundary = 'right'
  []
[]
...
[Materials]
  ...
  [constants]
    ...
    prop_values = '1.5     6.803e6          1.493e7                   ${GlobalParams/int_width}      '
...
[Outputs]
  ...
  perf_graph = true
  csv = true
  file_base = '${raw ${GlobalParams/file_base} ${GlobalParams/int_width} }'
[]
