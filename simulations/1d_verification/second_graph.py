...
print(density)
S_analy = 2*c0*density*math.sqrt(D*1e-8*3600/math.pi)
dM_analy = 2*c0*density*np.sqrt(D*1e-8*H*3600/math.pi)
ax.plot(sqrt_H, dM_analy,'k',linewidth=1.5 )

##YELLOWJACKET RESULT
n = 40
ax.plot(sqrt_H[1::n],dM[::n],'rd',markersize=3)

S,I = np.polyfit(sqrt_H[1:],dM,1)
#Force last point plot
ax.plot(sqrt_H[-1],dM[-1],'bd', zorder=10, clip_on=False,markersize=3)

print("Model error = ",(dM[-1] - dM_analy[-1] )*100/dM_analy[-1] )
...
ax.set_xlabel('$\sqrt{Time}$ ($hours^{1/2}$)',fontsize=7,fontweight='bold')
...
fig.savefig(PATH+'1d_verification_fig2.png',dpi=500, transparent=True)
