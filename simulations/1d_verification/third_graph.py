...
#Trendline
c = np.linspace(0,0.25,25)
c0 = c - 0.00177
density = 8.7e3 #mg/cm^3
V_a = 1.1131e-11*6.02214076e23*1e-12 #cm3/mol
density = 51.9961e3/V_a #mg/cm3
dM_analy = 2*c0*density*np.sqrt(3e-5*1e-8*3.6e6/math.pi)
P2, = ax.plot( c,dM_analy,'k-',linewidth=1.5,label="Analytical model") #,["",]
#Actual plot
P1, = ax.plot(c_Cr_plot,Y_c,'bd',markersize=3,label="Time = 1000 hours",)

S1,C1 = np.polyfit(c,dM_analy,1)
S2,C2 = np.polyfit(c_Cr_plot,Y_c,1)
...
xticks= np.around(np.linspace(xmin,xmax, 6),2)
...
ax.set_xlabel('Alloy Cr atom fraction',fontsize=7,fontweight='bold')
...
fig.savefig(PATH+'1d_verification_fig3.png',dpi=500, transparent=True)
